...
//TRANSMIT_REQUEST=
      ...
      if(espera>5000){
        ...
        espera++;
        if(teste>1800){teste=0;}
        if(teste2>50){teste2=0;}
        velocidade = teste;//(QEI1_SPEED_R*(float)7.32*1);//7.45
        torque_num=teste2;
        torque_num=teste2;
        torque_num=teste2;


      if((GPIO_PORTF_DATA_R & 0x08)==(0x08)){CONTROLE_ON=1;}else{CONTROLE_ON=0;}
    }
...
void Timer0A_Handler(void){
     ...
     if (CONTROLE_ON) {
     UART_OutString("RPM:");
     UART_OutUDec(velocidade);
     UART_OutString(",TORQUE:");
     UART_OutUDec(torque_num);
     UART_OutChar('\n');
     }else {
...
void Clock_Init(long SYSDIV_var){
 ...
 void PortF_Init(void){ volatile unsigned long delay;
     ...
     GPIO_PORTF_DIR_R = 0x06;          // PF1 E PF2 OUTPUT
...
// }
 ...
 void UART5_Init(void){
   ...
   UART5_IBRD_R        = 43;          // IBRD = int(80,000,000 / (16 * 115200)) = int(43,4027)
   UART5_FBRD_R        = 26;          // FBRD = int(0.4027 * 64 + 0.5) = 26,27777
   ...
   UART5_IFLS_R        = 0x00;        // 1/8 Cheia    // ira ativar o interrupt quando a fifo estiver 1/4 (0x08) cheia (0x0 para 1/8)
