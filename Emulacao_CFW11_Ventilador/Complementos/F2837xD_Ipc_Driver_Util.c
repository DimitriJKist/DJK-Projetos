//###########################################################################
// FILE:   F2837x_Ipc_Util.c
// TITLE:  F2837x Inter-Processor Communication (IPC) API Driver Utility
//         Functions
// DESCRIPTION:
...
IPCBootCPU2(uint32_t ulBootMode)
{
    ...
    // CPU01 to CPU02 INT1 interrupts. If CPU2 has already booted,
    // return a fail to let the application know that something is
    // out of the ordinary.
    while (1)
    {
    ...
    // control to CPU02 control system.
    switch (ulBootMode)
    {
		...
		case C1C2_BROM_BOOTMODE_BOOT_FROM_I2C:
		     ...
		     //Allows CPU2 bootrom to take control of clock configuration registers
             ClkCfgRegs.CLKSEM.all = 0xA5A50000;	
		     ClkCfgRegs.LOSPCP.all = 0x0002;
             EDIS;
